require 'pry'
class Dog

attr_accessor :id, :name, :breed

 def initialize(id=nil, options)
    @id = id
    @name = options[:name]
   @breed = options[:breed]
 end

 def self.create_table
  sql = <<-SQL 
   CREATE TABLE IF NOT EXISTS dogs (
   id INTEGER PRIMARY KEY,
   name TEXT,
   breed TEXT
      );
  SQL
  DB[:conn].execute(sql)
 end

 def self.drop_table
  sql = <<-SQL 
   DROP TABLE dogs;
  SQL
  DB[:conn].execute(sql)
 end

 def save
   if self.id
     self.update
   else
      sql = <<-SQL
      INSERT INTO dogs(name, breed)
      VALUES(?,?) 
      SQL
      DB[:conn].execute(sql, self.name, self.breed)
      @id = DB[:conn].execute("SELECT last_insert_rowid()FROM dogs")[0][0]
   end
     self
 end


  def self.create(options={})
    new_dog = Dog.new(options)
    new_dog.save
    new_dog
  end

  def self.find_by_id(id)
    sql = <<-SQL
      SELECT *
      FROM dogs
      WHERE id= ?
      LIMIT 1
      SQL
      DB[:conn].execute(sql, id).map do |row|
        self.new_from_db(row)
      end.first
      
  end

  def self.find_by_name(name)
    sql = <<-SQL
      SELECT *
      FROM dogs
      WHERE name= ?
      LIMIT 1
      SQL
      DB[:conn].execute(sql, name).map do |row|
        self.new_from_db(row)
      end.first
      
  end

  def self.new_from_db(row)
    
    row_hash = {name: row[1], breed: row[2]}
    new_dog = self.new(row[0], row_hash)
    # new_dog.id = row[0]
    # new_dog.name = row[1]
    # new_dog.breed = row[2]
    new_dog
     # create a new dog object given a row from the database
  end

   def self.find_or_create_by(options)
      
    dog = DB[:conn].execute("SELECT * FROM dogs 
      WHERE name=? AND breed =?", options[:name], options[:breed])
      if !dog.empty?
        dog_data =dog[0]
        dog_hash ={name: dog_data[1], breed: dog_data[2]}
        dog = Dog.new(dog_data[0], dog_hash)
      else
        dog = self.create(options)
      end
      
      dog
   end

  def update
    sql = "UPDATE dogs SET name=?, breed=? WHERE id=?"
    DB[:conn].execute(sql, self.name, self.breed, self.id)
  end


end